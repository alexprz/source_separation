...
for i in range(1,nb_iter):
    DJ = compute_gradient(B,y1,y2,x1,x2,lambda0)
    ...
    if(i==indice*100): # on affiche les images qui se "démélangent" toutes les 100 itérations et on recalcule la corrélation
        ...
        [Mat_or_cor_source] = correl_coef_composante_nb(s1,s2) # Calcul de la correlation entre les sources avant melange
        plt.pause(1)

        [Mat_mel_cor] = correl_coef_composante_nb(x1,x2) # Calcul de la correlation entre les sources melangees

        [Mat_sep_cor] = correl_coef_composante_nb(y1,y2) # Calcul de la correlation entre les sources separees
        plt.pause(5)
