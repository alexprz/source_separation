...
def correl_coef_composante_nb(im1_R,im2_R):
    ...
    for i in range(2):
        for j in range(2):
            prod_term = [(ima_R[k,i] - moy_R[i])*(ima_R[k,j] - moy_R[j]) for k in range(N)] # cette partie remplace le produit terme à terme de Matlab
            Mat_cor_R[i,j] = (1/(N*ec_R[i]*ec_R[j]))*sum(prod_term)
