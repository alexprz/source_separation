...
for i in range(10):
    ...
    A11 = 0.5 + i*0.05 # coefficients de mélange
    A12 = 1.-A11
    A21 = 0.5 - i*0.05
    A22 = 1.-A21
    ...
    corr11 = an.compute_bin_correlation(an.getBinaries(xx1), an.getBinaries(s1))
    corr22 = an.compute_bin_correlation(an.getBinaries(xx2), an.getBinaries(s2))
    corr21 = an.compute_bin_correlation(an.getBinaries(xx2), an.getBinaries(s1))
    corr12 = an.compute_bin_correlation(an.getBinaries(xx1), an.getBinaries(s2))

    maxcorr = max(max(corr11, corr22), max(corr21, corr12))
